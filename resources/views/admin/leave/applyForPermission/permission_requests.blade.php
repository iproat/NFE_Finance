										...
										@foreach($departmentlist AS $value)
											<tr>
												...
												<td>
													...
													->where('plant_approval_status','1')->where('department_approval_status','1')->where('employee_id',$value->employee_id)->where('status',1)->count();
                                                    @endphp
										...
										@foreach($plantlist AS $value)
											<tr>
												...
												@php
													...
													->where('plant_approval_status','1')->where('department_approval_status','1')->where('employee_id',$value->employee_id)->where('status',1)->count();
												@endphp
												@if(isset($checkpermissions)) {!! $checkpermissions !!}
												@else {!! '0' !!}
												 @endif
												</td>
												<td  style="width: 100px;"> 
												<a href="javacript:void(0)" data-status=1
                                            data-leave_application_id="{{ $value->leave_permission_id }}"
                                            class="btn remarksForPlantHead btn btn-rounded btn-success btn-outline m-r-5"><i
                                                class="ti-check text-success m-r-5"></i>@lang('common.approve')</a>
                                        <a href="javacript:void(0)" data-status=2
                                            data-leave_application_id="{{ $value->leave_permission_id }}"
                                            class="btn-rounded remarksForPlantHead btn btn-danger btn-outline"><i
                                                class="ti-close text-danger m-r-5"></i>@lang('common.reject')</a>                                   
												</td>										 
